...
function detectScreenshotAttempts(accountId?: string): () => void {
  ...
  const handleCopy = (e: ClipboardEvent) => {
    ...
    for (const item of items) {
      if (item.type.startsWith('image/')) {
        ...
        reporter.report({
          action: SECURITY_ACTIONS.CLIPBOARD_IMAGE_COPY,
          category: SECURITY_CATEGORIES.CLIPBOARD,
          result: 'WARNING',
  ...
  const handlePaste = (e: ClipboardEvent) => {
    ...
    if (totalSize > 1024 * 1024) {
      reporter.report({
        action: SECURITY_ACTIONS.LARGE_CLIPBOARD_COPY,
        category: SECURITY_CATEGORIES.CLIPBOARD,
        result: 'WARNING',
        details: `Large clipboard copy detected: ${(totalSize / 1024 / 1024).toFixed(2)}MB`
      }, accountId)
    }
  }
