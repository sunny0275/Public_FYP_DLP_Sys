...
export default function MfaSetupPage() {
  ...
  const loadMfaSetupRef = useRef(false)
  ...
  useEffect(() => {
    ...
    // If user is null, wait for persist storage to load before making any decisions
    if (!user) {
      return
    }

    if (!isSetupRequired && !hasValidSession) {      clearAuth()
    ...
    // Prevent double-load from StrictMode / user state updates
    if (loadMfaSetupRef.current) return
    loadMfaSetupRef.current = true
  ...
  useEffect(() => {
    ...
    if (countdown <= 0) {
      // Already handled by setTimeout in handleVerify
      return
    }
        ...
        {mfaData && (
          <div className="qr-code-container">
            ...
            <div>
              ...
              <div className="secret-code">{formatSecret(mfaData.secret)}</div>
            </div>
          </div>
        )}
