...
export default function DRMViewer({
  ...
  const logSecurityEvent = useCallback((
    ...
    if (isElectron()) {
      const baseUrl = (import.meta.env.VITE_API_BASE_URL || 'http://localhost:18080/api').replace(/\/$/, '')

      // Determine the correct endpoint URL
      const endpointUrl = baseUrl.includes('/api') ? `${baseUrl}/agent/endpoint/events` : `${baseUrl}/api/agent/endpoint/events`
  ...
  useEffect(() => {
      ...
      clipboardCheckInterval = setInterval(() => {
        if (navigator.clipboard && navigator.clipboard.read) {
          navigator.clipboard.read().then(clipboardItems => {
            ...
            const hasImage = Array.from(clipboardItems).some(item => item.types.includes('image/png') || item.types.includes('image/jpeg'))
            if (hasImage) {
    ...
    if (isElectron()) {
      ...
      recordingStopCleanup = setupRecordingStopListener((data) => {
        ...
        logSecurityEvent('SCREEN_RECORDING_STOP', 'WARNING', `Recording stopped: ${toolName} (duration: ${duration})`, 'Recording session ended')
      })
      ...
      electronCleanup = setupSuspiciousActivityListener((activity) => {
        ...
        if (typeof activity === 'string') {
          setSuspiciousActivity(prev => [...prev, `[System] ${activity}`])
        }
      })
    }
